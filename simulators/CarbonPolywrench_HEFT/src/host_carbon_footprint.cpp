/*
 * Copyright (c) 2024. The WRENCH Team and your name.
 * Este é o plugin de pegada de carbono para SimGrid/WRENCH.
 * Adaptado para SimGrid v3.36.
 */
...
private:
    ...
    double last_energy_joules_ = 0.0; // Gerenciamos o último valor de energia aqui
...
HostCarbonFootprint::HostCarbonFootprint(simgrid::s4u::Host* ptr) : host_(ptr) {
    ...
    last_energy_joules_ = sg_host_get_consumed_energy(host_); // Inicializa com o valor atual
}
...
void HostCarbonFootprint::update() {
    ...
    // CORREÇÃO: Usando a função pública e gerenciando o último valor internamente
...
void sg_host_carbon_footprint_plugin_init() {
    ...
    // CORREÇÃO: Usando o callback on_onoff_cb que substitui o on_state_change_cb
    simgrid::s4u::Host::on_onoff_cb(&on_host_change);
    simgrid::s4u::Host::on_speed_change_cb(&on_host_change);
    // CORREÇÃO: Removido o callback de Exec, pois on_speed_change_cb já cobre a mudança de carga.
}
